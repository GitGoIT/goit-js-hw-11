...
//////////////////////////////////////////////////////////////////////////
// async function onLoadMore() { // функція наступних догрузок результатів пошуку
//     loadMoreBtn.disable(); // встановлюємо кнопку в режим Loading... (метод винесений в зовнішній файл)
//     loadMoreBtn.show(); // показуємо кнопку завантаження (метод винесений в зовнішній файл)
//     const data = await newsApiService.fetchHits(); // викликаємо єдиний спільний метод роботи з API
  
//         const array = await data.hits
//         const markup = appendHitsMarkup(data.hits);
//         return markup;
...
function hitsTpl(hits) { // функцію шаблону мапінгу результатів фетчу в дом
    const markup = hits.map(hit => { // мапимо дані з API в заготовку html для картки, огортаємо зображення в лінк для підключення simplelightbox
